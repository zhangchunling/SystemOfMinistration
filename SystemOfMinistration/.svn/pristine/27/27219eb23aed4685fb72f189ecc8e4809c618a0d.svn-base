...
<!--  搜索表单  -->
<form id="form3" name='form3' action="<%=basePath %>q.servlet.action" enctype="multipart/form-data" method='get'>
...
<!--  内容列表   --><!--调用删除方法  -->
<form name="form2" id="form2" action="<%=basePath%>dMailBoxServlet.action" method="post">
...
<tr align="center" bgcolor="#FAFAF1" height="22">
	...
	<!--c标签的 循环  -->
				...
				<tr align='center' bgcolor="#FFFFFF" onMouseMove="javascript:this.bgColor='#FCFDEE';" onMouseOut="javascript:this.bgColor='#FFFFFF';" height="22" >
						...
						<td>${mes.userName}</td>
						<td>${mes.time }</td>
						<td><a href="<%=basePath%>up.servlet.action?id=${mes.id}">回复</a>|
						<a href="<%=basePath%>down.servlet.action?fileName=${mes.slave}">下载</a>|
						
						<a href="javascript:if(confirm('您真的忍心删除吗？'))window.location.
						 href='<%=basePath%>dMailBoxServlet.action?id=${mes.id}'">删除</a></td>
				</tr>		
				</c:forEach>
...
<td height="28" colspan="12">
	...
	<a href="<%=basePath %>ExcelServlet.action" class="coolbg">&nbsp;导出Excel&nbsp;</a>
</td>
</tr>
<tr align="right" bgcolor="#EEF4EA">
	<td height="36" colspan="12" align="center"><!--翻页代码 -->
		<%
    	 ...
    	 %>
    	<center>
	    	<a href="<%=basePath%>q.servlet.action?currentPage=1">首页</a>  
	    	<a href="<%=basePath%>q.servlet.action?currentPage=<%=upPage%>">上一页 </a> 
	    	<a href="<%=basePath%>q.servlet.action?currentPage=<%=nextPage%>">下一页</a>  
	    	<a href="<%=basePath%>q.servlet.action?currentPage=<%=mailBox.getTotalPage()%>">尾页</a>
    	</center>
	</td>
</tr>
</table>
