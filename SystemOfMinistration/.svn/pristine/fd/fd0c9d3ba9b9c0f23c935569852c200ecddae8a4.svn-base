...
public class Basic_Servlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		 ...
		 * 在点分页时，因为分页的链接里存了session里的关键字，如果移除session，则此时如果点分页的话，则链接里的值也为空;
		 * 而分页查询时获取的关键字就来自于分页链接里传的参数，这样将不是按搜索条件查回的数据进行分页，而是将所有数据分页（为空是查询所有）；
		 */
		
		//建立session
		HttpSession session=request.getSession();
		
		//点搜索时走的if：当页面有关键字参数时，	赋进session	
		if(keyword!=null&&!"".equals(keyword.trim())){					
			session.setAttribute("key", keyword);	//关键字的session没值的时侯，为其赋值				
		}
		//当点搜索时，传来的关键字为空时，去掉session里的值
		if(/*keyhid!=null&&Integer.parseInt(keyhid)==1&&*/keyword!=null&&"".equals(keyword.trim())){
