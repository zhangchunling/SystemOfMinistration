...
public class MailBoxDaoImpl extends JdbcUtil implements MailBoxDaoInter {
	...
	public void insertMailBox(MailBox mailbox) {// 数据库的增加方法
		String sql = "insert into mailbox (title,content,adressee,adresser,time,slave)values(?,?,?,?,?,?)";
		// 封装参数
	...
	public List<MailBox> queryMailBox(MailBox mailbox) {
			...
			StringBuffer sql = new StringBuffer("");//线程查询表里所有的内容
			
			if(mailbox.getAdressee()!=0)
				sql.append("select m.*,em.name name from mailbox m,employee em where  m.adressee=em.eid and adressee="+mailbox.getAdressee());
			else
				sql.append("select m.*,em.name name from mailbox m,employee em where  m.adresser=em.eid and adresser="+mailbox.getAdresser());
		...
		try {
			...
			while (rs.next()) {// 循环查找表里面的所有数据
				...
				mail.setUserName(rs.getString("name"));
				list.add(mail);
			}
			}
